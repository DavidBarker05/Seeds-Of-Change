...
public class Holdable : Interactable
{
    ...
    private bool AttemptToUnclip(int depth, LayerMask holdLayer, Camera cam)
    {
    ...
    private void UnblockLineOfSightToCamera(LayerMask holdLayer, Camera cam, int bitMask, ref Vector3 placePos)
    {
        float rayDistance = Vector3.Distance(cam.transform.position, transform.position);
        if (Physics.Raycast(cam.transform.position, cam.transform.forward, out RaycastHit hit, rayDistance, bitMask)) // Something is blocking line of sight from the camera's forward to this holdable object
        {
            isClipping = true;
            if (Physics.Linecast(cam.transform.position, transform.position, out RaycastHit boundHit, holdLayer)) // Hit on bounds of this holdable closest to camera
            {
                ...
                placePos = cam.transform.position + cam.transform.forward * (hit.distance - boundsLength); // Place this holdable object closer to the camera on the other side of the surface, place the bounds on the face of the surface to stop overlapping
            }
        }
    }
